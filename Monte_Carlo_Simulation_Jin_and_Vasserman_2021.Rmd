...
sim_lambda_expand <- gen_lambda(df_t0, cost_par, L)
...
#tmp_score <- sim_renewal_price(df_t0, theta, cost_par, R, sim_lambda_expand)

# Calculate Choice Probability
cal_v_modified <- function(df, theta, cost_par, R, lambda){
 # Parameters
  eta_f <- theta[1:4]
  xi <- theta[5:6]
  alpha_m0 <- theta[7:9]
  ...
  # Price
  price <- sim_renewal_price(df, theta, cost_par, R, lambda)
...
cal_choice_probability_modified <- function(df, theta, cost_par, R, lambda){
  ## Calculate v
  v <- cal_v_modified(df, theta, cost_par, R, lambda)
...
tmp_choice_prob <- cal_choice_probability_modified(df_t0, theta, cost_par, R, sim_lambda_expand)
```
